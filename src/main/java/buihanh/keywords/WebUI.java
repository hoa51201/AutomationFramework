...
/**
 * Keyword WebUI là class chung làm thư viện xử lý sẵn với nhiều hàm custom từ
 * Selenium và Java. Trả về là một Class chứa các hàm Static. Gọi lại dùng bằng
 * cách lấy tên class chấm tên hàm (WebUI.method)
 */
public class WebUI {
	...
	public static final String SCREENSHOT_ALL_STEPS = "NO";
	...
	public static void addScreenshotToReport(String screenshotName) {
		if (SCREENSHOT_ALL_STEPS.equals("NO")) {
	...
	public static void disableinfobars() {
		...
		// Tạo các tùy chọn trình duyệt để tắt quảng cáo 
		ChromeOptions options = new ChromeOptions(); 
		options.addArguments("--disable-extensions"); 
		// Vô hiệu hóa các tiện ích mở rộng 
		DesiredCapabilities capabilities = new DesiredCapabilities();
		capabilities.setCapability(ChromeOptions.CAPABILITY, options);
		options.merge(capabilities);
		options.addArguments("--disable-popup-blocking"); 
		// Vô hiệu hóa chặn popup 
		options.addArguments("--disable-infobars"); 
		options.addArguments("disable-popup-blocking");
		options.addArguments("incognito");
		// Vô hiệu hóa thanh thông tin trình duyệt 
	}
	...
	public static void getToUrlAuthentication(String url, String username, String password) {
		...
		DevTools devTools = ((HasDevTools) DriverFactory.getDriver()).getDevTools();
	...
	public static void setWindowSize(int widthPixel, int heightPixel) {
		DriverFactory.getDriver().manage().window().setSize(new Dimension(widthPixel, heightPixel));
	}
	...
	public static void setWindowPosition(int X, int Y) {
		DriverFactory.getDriver().manage().window().setPosition(new Point(X, Y));
	}
	...
	public static void minimizeWindow() {
		DriverFactory.getDriver().manage().window().minimize();
	}
	...
	/**
	 * In trang hiện tại trong trình duyệt. Note: Chỉ hoạt động ở chế độ headless
	 *
	...
	public static String printPage(int endPage) {
		...
		Pdf pdf = ((PrintsPage) DriverFactory.getDriver()).print(printOptions);
	...
	/**
	 * Chuyển đổi đối tượng dạng By sang WebElement Để tìm kiếm một element
	 *
	 * @param by là element thuộc kiểu By
	...
	/**
	 * Chuyển đổi đối tượng dạng By sang WebElement Để tìm kiếm nhiều element
	 *
	 * @param by là element thuộc kiểu By
	...
	public static ChromeOptions notificationsAllow() {
		...
		// Dùng hàm setExperimentalOption thực thi giá trị thông qua đối tượng prefs
		// trên
	...
	public static String getCurrentUrl() {
		smartWait();
		Log.info("Get Current URL: " + DriverFactory.getDriver().getCurrentUrl());
	...
	public static String getPageTitle() {
		...
		Log.info("Get Page Title: " + DriverFactory.getDriver().getTitle());
		return title;
	}
	...
	public static void switchToFrameByIndex(int index) {
		...
		WebDriverWait wait = new WebDriverWait(DriverFactory.getDriver(), Duration.ofSeconds(WAIT_EXPLICIT),
	...
	public static void switchToWindowOrTab(int position) {
		...
		DriverFactory.getDriver().switchTo()
				.window(DriverFactory.getDriver().getWindowHandles().toArray()[position].toString());
	}

	public static boolean verifyNumberOfWindowsOrTab(int number) {
		return new WebDriverWait(DriverFactory.getDriver(), Duration.ofSeconds(WAIT_EXPLICIT))
	...
	public static void switchToMainWindow() {
		sleep(WAIT_SLEEP_STEP);
		DriverFactory.getDriver().switchTo()
				.window(DriverFactory.getDriver().getWindowHandles().toArray()[0].toString());
	}
	...
	public static void switchToLastWindow() {
		...
		DriverFactory.getDriver().switchTo()
				.window(DriverFactory.getDriver().getWindowHandles().toArray()[windowHandles.size() - 1].toString());
	}
	...
	public static boolean verifyElementTextEquals(By by, String text, FailureHandling flowControl) {
        ...
        if (flowControl.equals(FailureHandling.CONTINUE_ON_FAILURE)) {
            ...
            if (result == false) {
                Log.error("The actual text is '" + getTextElement(by).trim() + "' not equals '" + text.trim() + "'");
            }
        }
		return getTextElement(by).trim().equals(text.trim());
...
//				Thread.currentThread().getStackTrace()[1].getMethodName() + "_" + DateUtils.getCurrentDateTime());
		return result;
	}

	/*
	 * @Step("Verify text of an element [Contains]") public static boolean
	 * verifyElementTextContains(By by, String text, FailureHandling flowControl) {
	 * smartWait(); waitForElementVisible(by);
	 * 
	 * boolean result = getTextElement(by).trim().contains(text.trim());
	 * 
	 * if (result == true) { Log.info("Verify text of an element [Contains]: " +
	 * result); } else { Log.warn("Verify text of an element [Contains]: " +
	 * result); }
	 * 
	 * if (flowControl.equals(FailureHandling.STOP_ON_FAILURE)) {
	 * Assert.assertTrue(result, "The actual text is " + getTextElement(by).trim() +
	 * " not contains " + text.trim()); } if
	 * (flowControl.equals(FailureHandling.CONTINUE_ON_FAILURE)) {
	 * softAssert.assertTrue(result, "The actual text is " +
	 * getTextElement(by).trim() + " not contains " + text.trim()); } if
	 * (flowControl.equals(FailureHandling.OPTIONAL)) { if
	 * (ExtentTestManager.getExtentTest() != null) {
	 * ExtentReportManager.warning("Verify text of an element [Contains] - " +
	 * result); }
	 * AllureManager.saveTextLog("Verify text of an element [Contains] - " +
	 * result); }
	 * 
	 * addScreenshotToReport(Thread.currentThread().getStackTrace()[1].getMethodName
	 * () + "_" + DateUtils.getCurrentDateTime());
	 * 
...
//				Thread.currentThread().getStackTrace()[1].getMethodName() + "_" + DateUtils.getCurrentDateTime());
	...
	public static boolean dragAndDropHTML5(By fromElement, By toElement) {
		...
		try {
			...
			// Chổ này lấy toạ độ hiện tại cộng thêm 120px là phần header của browser
			// (1920x1080 current window)
	...
	public static void reloadPage() {
		...
		waitForPageLoaded();
		Log.info("Reloaded page " + DriverFactory.getDriver().getCurrentUrl());
	}
	...
	/**
	 * Kiểm tra giá trị từng cột của table khi tìm kiếm theo điều kiện CHỨA với
	 * xpath tuỳ chỉnh
	 *
	 * @param column           vị trí cột
	 * @param value            giá trị cần so sánh
	...
	public static void checkContainsValueOnTableByColumn(int column, String value, String xpathToTRtagname) {
		...
		// xpathToTRtagname is locator from table to "tr" tagname of data section:
		// //tbody/tr, //div[@id='example_wrapper']//tbody/tr, ...
		List<WebElement> totalRows = DriverFactory.getDriver().findElements(By.xpath(xpathToTRtagname));
		sleep(1);
		Log.info("Number of results for keywords (" + value + "): " + totalRows.size());
	...
	public static ArrayList getValueTableByColumn(int column) {
		...
		List<WebElement> totalRows = DriverFactory.getDriver().findElements(By.xpath("//tbody/tr"));
		sleep(1);
		Log.info("Number of results for column (" + column + "): " + totalRows.size()); // Không thích ghi log thì xóa
																						// nhen
		...
		if (totalRows.size() < 1) {
			...
			for (int i = 1; i <= totalRows.size(); i++) {
				...
				WebElement title = DriverFactory.getDriver()
						.findElement(By.xpath("//tbody/tr[" + i + "]/td[" + column + "]"));
	...
	public static boolean verifyElementAttributeValue(By by, String attribute, String value) {
		...
		try {
			...
			Log.error("Object: " + by.toString() + ". Not found value: " + value + " with attribute type: " + attribute
					+ ". Actual get Attribute value is: " + getAttributeElement(by, attribute));
			Assert.fail("Object: " + by.toString() + ". Not found value: " + value + " with attribute type: "
					+ attribute + ". Actual get Attribute value is: " + getAttributeElement(by, attribute));
			return false;
		}
	}
	...
	//Kiểm tra giá trị tìm kiếm trong bảng
	public static void checkDataTableByColumn(int column, String value) {
		WebUI.waitForPageLoaded();
		WebUI.sleep(2);
		// Xác định số dòng của table sau khi search
		List<WebElement> row = WebUI.getWebElements(By.xpath("//table//tbody/tr"));
		int rowTotal = row.size(); // Lấy ra số dòng
		Log.info("The number of records found: " + rowTotal);
		if (rowTotal < 1) {
			Log.info("Not found value: " + value);
		}
		else {
			...
			for (int i = 1; i <= rowTotal; i++) {				
				...
				String colName = elementCheck.getText();
				if(value.toUpperCase().equals(colName.toUpperCase())) {
					Log.info(value + " - " + "data exists!");
				}
				else {
					Log.info("Record" + i + " not contains" + value);
				}
			}
		}
		
	}
	//Kiểm tra giá trị tìm kiếm trong bảng và click xem chi tiết bản ghi đó
		public static void checkAndClickDataTable(int column, String value) {
			WebUI.waitForPageLoaded();
			WebUI.sleep(2);
			// Xác định số dòng của table sau khi search
			List<WebElement> row = WebUI.getWebElements(By.xpath("//table//tbody/tr"));
			...
			if (rowTotal < 1) {
				...
				//duyệt từng dòng
				for (int i = 1; i <= rowTotal; i++) {				
					WebElement elementCheck = WebUI.getWebElement(By.xpath("//table//tbody/tr[" + i + "]/td[" + column + "]/a"));
					WebUI.scrollToElement(elementCheck);
					//String colName = elementCheck.getText();
					if(value.toUpperCase().equals(elementCheck.getText().toUpperCase())) {
						Log.info(value + " - " + "data exists!");
						elementCheck.click();
						Log.info("click record: "+ value);
					}
					else {
						Log.info("Record " + i + " not equals " + value);
					}
				}
			}
	...
	//Tìm kiếm dữ liệu và click xoá
	public static void clickDeleteTableByColumn(int column1, int column2, String value) {
		WebUI.waitForPageLoaded();
		WebUI.sleep(2);
		...
		List<WebElement> row = WebUI.getWebElements(By.xpath("//table//tbody/tr"));
		int rowTotal = row.size(); // Lấy ra số dòng
		System.out.println("The number of records found: " + rowTotal);
		// Duyệt từng dòng
		if (rowTotal < 1) {
			Log.info("Not found value: " + value);
		}
			...
			for (int i = 1; i <= rowTotal; i++) {
				//// tbody/tr[1]/td[9]/a[2]
				WebElement elementCheck = WebUI.getWebElement(By.xpath("//table//tbody/tr[" + i + "]/td[" + column1 + "]"));
				WebUI.scrollToElement(elementCheck);
				String colName = elementCheck.getText();
				if (value.toUpperCase().equals(colName.toUpperCase())) {
					Log.info(value + " - " + "data exists!");
		            WebElement elementDelete = WebUI.getWebElement(By.xpath("//table//tbody/tr[" + i + "]/td[" + column2 + "]/a[2]"));
		            WebUI.scrollToElement(elementDelete);
					elementDelete.click();
				}
				else {
					Log.error("Record" + i + " not contains" + value);
				}
			}
		}
